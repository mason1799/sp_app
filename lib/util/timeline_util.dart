...
class TimelineUtil {
  ...
  static String format(
    ...
    String timeline;
    if (_info.customYesterday().isNotEmpty && DateUtil.isYesterdayByMs(ms, _locTimeMs)) {
    ...
    if (seconds < 90) {
      ...
      if (suffix != _info.suffixAfter() && _info.lessThanOneMinute().isNotEmpty && seconds < _info.maxJustNowSecond()) {
  ...
  ///0天12:13:59 or 12:13:59
  static String constructEndTime(int seconds) {
    int day = seconds ~/ 3600 ~/ 24;
    int hour = seconds ~/ 3600;
    int minute = seconds % 3600 ~/ 60;
    int second = seconds % 60;
    if (day != 0) {
      return '$day天${formatTime(hour)}:${formatTime(minute)}:${formatTime(second)}';
    } else {
